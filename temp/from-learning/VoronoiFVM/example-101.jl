...
# ╔═╡ ac5e2a26-6587-49d5-a542-16b1f79787a7
md"""
# 101: 1D Laplace equation 
"""

# ╔═╡ a55e9126-534b-4e64-9a4d-4551666952c4
md"""
## Import tools
"""

# ╔═╡ a4ffede0-3517-11ee-04c5-d7af66c55142
import VoronoiFVM as FVM

# ╔═╡ 6affd9f4-d0e4-4d11-b1fc-2fe5b17dd45e
md"""
## Problem definition
"""

# ╔═╡ 19ec45eb-0121-442f-baa8-9ae42d74651c
md"""
...
We replace the Dirichlet boundary condition by a Robin boundary condition with a penalty $\varepsilon^{-1}$:
...
This penalty method for the implementation of Dirichlet boundary conditions is used throughout VoronoiFVM. In order to discretize it, we choose collocation points $\gamma_1=x_1 < x_2 < \dots < x_n=\gamma_2$. For instance, we can choose 6 collocation points in $(0,1)$: from these, we create a discretization grid structure for working with the method.

This implicitly creates a number of control volumes $\omega_k $ around each
discretization point $x_k$: Let $\sigma_{k,k+1}=\frac{x_k+x_{k+1}}{2}$. Then $\omega_1=(\gamma_1,\sigma_{1,2})$, $\omega_k= (\sigma_{k-1,k}, \sigma_{k,k+1})$ for $k=2\dots n-1$, $\omega_{n}=(\sigma_{n-1,n},\gamma_2)$. 
...
This domain discretization is provided as follows:
"""

# ╔═╡ 1f4c4290-3e7b-4519-8670-01b0ae7ecfeb
begin
	n = 6
	...
	grid = FVM.Grid(γ₁:δγ:γ₂)
end

# ╔═╡ 42e26ffc-3366-4603-949d-35e909013c5b
md"""
...
0 & = \int_{\omega_k} -\Delta u d\omega= -\int_{\partial \omega_k}\nabla u ds\\[12pt]
  & = \begin{cases}
	  u'(\sigma_{1,2}) - u'(0)& k=1\\[6pt]
	  u'(\sigma_{k,k+1}) - u'(\sigma_{k-1,k}) & 1<k<n\\[6pt]
	  u'(1)- u'(\sigma_{n,n+1})&k=n
	  \end{cases}\\[12pt]
  & \approx\begin{cases}
	\dfrac{1}{x_2-x_1} g(u_1,u_2) + \dfrac{1}{\varepsilon}(u_1-0)& k=1\\[6pt]
	\dfrac{1}{x_k-x_{k-1}}g(u_k,u_{k-1}) -\dfrac{1}{x_{k+1}-x_{k}}g(u_{k+1},u_{k}) & 1<k<n\\[6pt]
	\dfrac{1}{\varepsilon}(u_n-1)+ \dfrac{1}{x_n-x_{n-1}} g(u_{n},u_{n-1})&k=n
	\end{cases}
\end{aligned}
```
...
The required interior fluxes are computed with `flux!` provided below:
"""

# ╔═╡ 1b732832-7762-4aec-b8bc-a4325316e447
function flux!(f, u, edge)
	"""
		flux!(f, u, edge)
	
	Flux between neighboring control volumes.
	"""
	f[1] = u[1, 1] - u[1, 2]
end

# ╔═╡ 69574a07-9e3d-41e2-940e-2007cfbfcd0f
md"""
### Boundary conditions

Next, we write a function `bcond!` describing $g$, we create a physics record, and a finite volume system with one unknown species and a dense matrix to describe it's degrees of freedom (the matrix used  to calculate the solution is sparse). We give the species the number 1 and enable it for grid region number one 1. Then, we set boundary conditions for species 1 at $\gamma_1, \gamma_2$.
"""

# ╔═╡ 2cf7ebb8-1409-45ff-938c-a67771ecea68
function bcond!(args...)
	"""
		bcond!(args...)

	Set boundary conditions in different regions.
	"""
	FVM.boundary_dirichlet!(args...; region = 1, value = 0)
	FVM.boundary_dirichlet!(args...; region = 2, value = 1)
end

# ╔═╡ 82c20d1a-b7e9-46b7-a6cf-0b1e239aa52d
md"""
## Solution

Create system with the one dimensional discretization grid previously allocated. This will handle the flux function and boundary conditions. We set a zero initial value.
"""

# ╔═╡ 809983d5-8ff1-47ce-b56e-ae4242a4f0a1
sys = FVM.System(
	grid;
	flux      = flux!,
	breaction = bcond!,
...
# ╔═╡ 25a82d6a-275f-4636-8d1e-7799b6976e84
md"""
With these data, we solve the system.
"""

# ╔═╡ 50428951-3570-4f67-9382-993b586eb697
solution = FVM.solve(sys; inival = 0)

# ╔═╡ b756ae06-40d0-452d-9e2c-1a167ac6d1da
md"""
Test value provided by original example: $(sum(solution) ≈ 3.0 ? "PASS" : "FAIL").
"""
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "68ff27389ebf0b41c0b4a1e2de0ed07f76447862"
