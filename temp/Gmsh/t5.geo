...
Macro CheeseHole
    ...
    // We then store the surface loops tags in a list for later reference (we
    // will need these to define the final volume):
    theloops[t] = newsl;
    Surface Loop(theloops[t]) = {s1, s2, s3, s4, s5, s6, s7, s8};

    // We define a physical volume for each hole (if meshing the wholes!)
    // thehole = newv;
...
For t In {1:5}
    ...
    Printf("Hole %g (center = {%.3f,%.3f,%.3f}, radius = %g)!",
        t, x, y, z, r);
EndFor
...
// To generate a curvilinear mesh and optimize it to produce probably valid
