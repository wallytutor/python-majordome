...
# ╔═╡ b4274ba6-2814-44c8-99cb-d27af8f1b405
begin
	using CairoMakie
	using Symbolics
...
# ╔═╡ 1d5adee2-cead-4810-8594-cf550141a025
md"""
# Function approximation
"""

# ╔═╡ ac455ad0-ade0-11ee-1da8-b364410f1eb4
md"""
## Least squares method
"""

# ╔═╡ c84ed32f-d465-49c5-91e2-abc5b59be8f4
function innerproduct(f, g, x, Ω; symbolic=false)
	p = integrate(f * g, (x, Ω...); symbolic, detailed=false)
	return isa(p, Num) ? p.val : p
end

# ╔═╡ b751d801-d852-4099-9d7d-ac9156e7dab3
function least_squares_1d(f, ψ, Ω; symbolic=true)
	z = Symbolics.get_variables(f)[1]
	...
	for (i, ψᵢ) in enumerate(ψ)
		for (j, ψⱼ) in enumerate(ψ)
			A[i, j] = innerproduct(ψᵢ, ψⱼ, z, Ω; symbolic)
		end
		
		b[i] = innerproduct(ψᵢ, f, z, Ω)
	end
...
# ╔═╡ 2877493c-7166-45ec-b705-92eceb11f246
function regression_1d(f, ψ, Ω; samples = 100)
	z = range(Ω..., samples)
	x = Symbolics.get_variables(f)[1]
	F = build_function(f, x; expression = Val{false}).(z)
	Ψ = [build_function(ψₖ, x; expression = Val{false}) for ψₖ in ψ]
	
	N = length(Ψ) - 1
	m = length(z)

	B = zeros(Number, N+1, N+1)
	d = zeros(Number, N+1)
...
# ╔═╡ eec6abfb-d20f-45ab-b224-eef36db82573
function plot_approx_1d(f, g, Ω; samples = 100)
	z = range(Ω..., samples)
	x = Symbolics.get_variables(f)[1]
	F = build_function(f, x; expression = Val{false})
	G = build_function(g, x; expression = Val{false})

	kwargs = (; xgridstyle = :dot,   ygridstyle = :dot, 
	            xgridwidth =  0.5,   ygridwidth = 0.5,
	            xgridcolor = :black, ygridcolor = :black)
	
	fig = Figure(size = (700, 400))
	ax = Axis(fig[1, 1]; xlabel = "x", ylabel = "y", kwargs...)
	lines!(ax, z, F.(z); color = :black, label = "Exact", linewidth = 4)
	lines!(ax, z, G.(z); color = :red,   label = "Approximation")
	axislegend(ax; position = :lt)

	return fig, ax
end

# ╔═╡ b2ad89ff-8fe1-402a-ab9d-f9ced5bb5c08
function example01(approx, x, ψ)
	Ω = [1, 2]
	
	f = 10 * (x - 1)^2 - 1
	g = approx(f, ψ, Ω)
	
	fig, ax = plot_approx_1d(f, g, Ω)
	ax.xticks = 1:0.2:2
	xlims!(ax, 1, 2)
...
# ╔═╡ 1334ab5f-cbd3-46c6-9ebd-bed8764fb49f
let
	@variables x::Real
	example01(least_squares_1d, x, [1, x])
end

# ╔═╡ d961d7fa-da6a-48c5-9716-927af9d01559
let
	@variables x::Real
	example01(least_squares_1d, x, [1, x, x^2])
end

# ╔═╡ 1c7c6910-355d-498c-b015-bdecb7d154f5
md"""
## Regression method
"""

# ╔═╡ 21ab1166-21f5-4c15-a276-a9fc1671ac79
let
	@variables x::Real
	example01(regression_1d, x, [1, x])
end

# ╔═╡ 6163fbd6-e589-418c-bb3a-a71067c320d9
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "b612e5c3d3ffeece7ad897b79fc69478dcd6e1b4"
...
git-tree-sha1 = "ab9e65a1a0cfa0c1add8525f75533aea063cd8b4"
...
deps = ["DataDrivenDiffEq", "DataDrivenSparse", "DataStructures", "LinearAlgebra", "SpecialFunctions", "Statistics", "SymbolicUtils", "Symbolics"]
git-tree-sha1 = "e45b3e33b70ed0454dbd59f27507b5800c5ee895"
