...
- Maxwell-Garnett approximation (`EffectiveThermalConductivity.maxwell_garnett`) as exposed in [@Hanein2017]; further discussion of its origin and applicability is provided by [@Kiradjiev2019].

- For porous media (packed bed in the context) with enhanced radiative effects the model by [@Singh1994] as discussed by [@Kee2017] is implemented  (`EffectiveThermalConductivity.singh1994`).
...
print(f"First layer thickness: {1e6*y_first:.0f} µm")
```

```{python}
doc = DocumentedClass(WallGradingCalculator)
doc.constructor()
doc.members()
doc.classmethods()
doc.staticmethods()
...
```{python}
doc = DocumentedClass(SkinFrictionFactor)
doc.constructor()
doc.staticmethods()
```
