...
```{python}
%%md
Now we can also talk about the function we just defined: {f_x}! If you need to inject the expression inside a math block, you can do so by calling `sp.latex` and using the string formatter: $f={sp.latex(f_x):s}$. It should also work with block math:
