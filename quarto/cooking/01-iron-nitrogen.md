...
from majordome import Capturing
...
class SystemCalculator:
    ...
    def __call__(self, *args, **kwargs):
        return self._ce(*args, **kwargs)

    def equilibrate(self, T, w, guess=None):
        results = self._ce(T, w, guess=guess)
        return results[0]["x"].full().flatten()
```
...
class NitridingLayer:
    ...
    def fraction_alpha(T, u, guess):
        ...
        return np.array([CE.equilibrate(T, uk, guess=guess[k])
    ...
    def _build(self, t_end, x_end, u0, T, **kwargs):
        ...
        self._guess = np.array([CE.equilibrate(T(0), uk) for uk in u0])
