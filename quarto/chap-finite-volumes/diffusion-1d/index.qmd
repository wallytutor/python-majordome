...
```{python}
J_w = -FaceVars.D_w * (CellVars.c_P - CellVars.c_W) / FvmVars.delta
J_e = -FaceVars.D_e * (CellVars.c_E - CellVars.c_P) / FvmVars.delta
...
```{python}
lhs = expand(lhs * FvmVars.tau / FvmVars.delta)
rhs = expand(rhs * FvmVars.tau / FvmVars.delta)
...
```{python}
eq = Eq(eq.subs(diffusion.SUBS_FOURIER), 0)
eq
```
